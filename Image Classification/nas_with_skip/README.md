...
A PyTorch implementation of **Neural Architecture Search with Reinforcement Learning** (Zoph & Le, 2017), applied to CIFAR-10. A recurrent controller (LSTM) samples child network architectures, trains them, and uses their validation accuracy as a reward signal to improve itself via REINFORCE.
